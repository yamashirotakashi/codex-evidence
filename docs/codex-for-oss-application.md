...
```text
codex-evidenceは、個人のローカルなセッション運用から、Codexとの共同作業でOSSメンテナー向けに抽出・再設計したlocal-first evidence toolkitです。current-state/handoff/ログ断片をSQLiteに取り込み、検索可能なevidence_cardと再開用context-packへ圧縮します。技術書編集で重視する構造化・検証・再現性を保守文脈へ適用し、公開hygiene gateとdogfood proofで個人履歴を読まない境界も確認済みです。
```
...
```text
技術書編集者として、複雑な技術知識を構造化し、読者が再現できる形に整える仕事をしてきました。この申請は、その経験をCodexと組み合わせ、OSSメンテナンスに必要な文脈保持、証跡化、再開性へ適用する実証です。私的な作業メモから公共性のあるツールを抽出し、仕様化、実装、公開前衛生検査までCodexと反復しました。未実装のPR自動レビューや広域自動化は主張しません。
```
